...
function OptimizationForSubmitText(mytext)
{
   ...
   $("#chatbox"+$("#hiddenUserId").val()).children(".OnlyText").html(mytext);  
...
//alert( $("#"+$("#hiddenUserId").val()).css('background-position'));
   $.ajax({
          ...
          data:"{'roomId':'" + GetRoomId() + "','backPos':'" + $("#"+$("#hiddenUserId").val()).css('background-position') + "','top':'" + parseInt(e.pageY-32) + "','left':'" + parseInt( e.pageX-32) + "','userText':'" + userText + "'}",
          contentType: "application/json; charset=utf-8",
          dataType: "json",
          success: function(msg) {
...
//alert( $("#"+$("#hiddenUserId").val()).css('background-position'));
   $.ajax({
          ...
          type: "POST",
          url: "club.aspx/UpdateMyAvatar",
          data:"{'roomId':'" + GetRoomId() + "','backPos':'" + $("#"+$("#hiddenUserId").val()).css('background-position') + "','top':'" + parseInt($("#"+$("#hiddenUserId").val()).css('top')) + "','left':'" + parseInt($("#"+$("#hiddenUserId").val()).css('left')) + "','userText':'" + userText + "'}",
          contentType: "application/json; charset=utf-8",
          dataType: "json",
          success: function(msg) {
                    GenericChatterList=msg.d; 
...
function SetMyFaceDirection(e){
      ...
      var OutX=parseInt(e.pageX -$("#"+$("#hiddenUserId").val()).css('left').toString().split("px")[0]);
      var OutY=parseInt(e.pageY -$("#"+$("#hiddenUserId").val()).css('top').toString().split("px")[0]);
      var bTop=0;
      $("#tester").text("(Outx:"+OutX+")  ("+ "OutY:"+OutY+")" );
        ...
        var backgroundLeft= parseInt($("#"+$("#hiddenUserId").val()).css('background-position').toString().split(" ")[0].toString().split('px')[0]);
...
function DisplayAvatarDetails(e,userid,displayName){
   $("div#avatar-details").fadeIn('fast');     
     if(userid==parseInt($("#hiddenUserId").val()))
     {
        $("#profilePhoto").fadeIn();
                ...
                for (var i in GenericChatterList)
                 {
                    ...
                    if($("#"+Chatter.Id).length<1) 
                    {
                        ...
                        if(IsThisPointIsInsideTheClubAreas(parseInt(Chatter.Left),parseInt(Chatter.Top))!=-1)
                        {
...
//                        $(avat).css('background-color',"blue");
                        ...
                        //
//                            var transparentImage=$('<img/>');
//                            $(transparentImage).attr('src','Images/site/callout33.gif');
//                            $(transparentImage).addClass('chatBackGround');
//                            $(ChatText).append(transparentImage);
   ...
   function MoveAvatar(Chatter){
    ...
    $("#"+Chatter.Id).css('background-image',Chatter.Avatar);
      ...
      var OutX=parseInt(Chatter.Left.toString().split("px")[0]-$("#"+Chatter.Id).css('left').toString().split("px")[0]);
      var OutY=parseInt(Chatter.Top.toString().split("px")[0]-$("#"+Chatter.Id).css('top').toString().split("px")[0]);
      var bTop=0;
      $("#tester").text("(Outx:"+OutX+")  ("+ "OutY:"+OutY+")" );
        ...
        if((OutX>0 && OutY==0)||(OutX>0 && Math.abs(OutY)<16)){
         ...
         bTop=192; // center to right bottom
           ...
           bTop=256; //center to left bottom 
            ...
            var x1=parseInt( $("#"+Chatter.Id).css('left').toString().split("px")[0]);
...
//          
              ...
              var y1=parseInt( $("#"+Chatter.Id).css('top').toString().split("px")[0]);
              var x2=parseInt(Chatter.Left.toString().split("px")[0]);
              var y2=parseInt(Chatter.Top.toString().split("px")[0]);
               var m=1;
...
var y=parseInt( (x-x1)*(y2-y1)/(x2-x1)+ y1);
if(  IsThisPointIsInsideTheClubAreas(x+32,y+32)>-1)
  {  
    if(  IsThisPointIsInsideTheClubAreas(x+32,y+32)==0)
    {
         if( $("#hiddenUserId").val()==Chatter.Id)
            {
              StopMeHere();
//              $("#"+Chatter.Id).fadeOut('slow');
        ...
        return;
           ...
           if( $("#hiddenUserId").val()==Chatter.Id)
            {
              $("#hiddenRoomId").val(IsThisPointIsInsideTheClubAreas(x+32,y+32));
...
if(isNaN(y)==false)
{
    ...
    var bLeft= parseInt($("#"+Chatter.Id).css('background-position').toString().split(" ")[0].toString().split('px')[0]);
     if(bLeft==256){
        bLeft=64;
     }
     else{
 ...
 for (var i in ClubPlaceList)
                 {
                     ...
                     var yList=new Array();
                        ...
                        {
                            xList[$(xList).length]= parseInt( ClubPlaceList[i].Xs[j]);
                            yList[$(yList).length]=parseInt( ClubPlaceList[i].Ys[j]);
                            var folderP=$('<div></div>');
 ...
 var yList=clubarea.Ys;
  ...
  var i;
        ...
        var  oddNodes=false;
          ...
          {
                if (parseInt(yList[i])<y && parseInt(yList[j])>=y ||  parseInt(yList[j])<y && parseInt(yList[i])>=y)
                 {
                  if (parseInt(xList[i])+(y-parseInt(yList[i]))/(parseInt(yList[j])-parseInt(yList[i]))*(parseInt(xList[j])-parseInt(xList[i]))<x)
                   {
                    oddNodes=!oddNodes;
                   }
                 }     
...
$(document).ready(function(){
    ...
    $(".ShowHideMap,div#hide-map,div#ChatLocations").click(function(){
