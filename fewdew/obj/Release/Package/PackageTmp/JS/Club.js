...
function OptimizationForSubmitText(mytext)
{
   ...
   //$("#chatbox"+$("#hiddenUserId").val()).text(mytext);
                ...
                for (var i in GenericChatterList)
                 {
                    ...
                    if($("#"+Chatter.Id).length<1) 
                    {
                        ...
                        if(IsThisPointIsInsideTheClubAreas(parseInt(Chatter.Left,10),parseInt(Chatter.Top,10))!=-1)
                        {
                           var tempLeft=450; 
                           var tempTop=250;
                           while(IsThisPointIsInsideTheClubAreas(tempLeft,tempTop)!=-1)
                           {
                            tempTop=tempTop+10;
                           }
                           
                           $(avat).css('left',tempLeft+"px");
                           $(avat).css('top',tempTop+"px");
                            
                           //$("#publicFeed").html($(avat).css('left'));
                        }
                        else{
                            $(avat).css('left',Chatter.Left);
                            $(avat).css('top',Chatter.Top);
                            //$("#publicFeed").html(Chatter.Left);
...
//                        $(ChatText).append(OnlyText);
                    ...
                    if(Chatter.Message=="undefined"){
                        ...
                        if($("#chatbox"+Chatter.Id).html().toUpperCase()!=Chatter.Message.toUpperCase())
                        {
                          //alert($("#chatbox"+Chatter.Id).children(".OnlyText").html().toUpperCase()+" "+Chatter.Message.toUpperCase()); 
                            ...
                            $("#chatbox"+Chatter.Id).css('left',parseInt( $("#"+Chatter.Id).css('left').split("px")[0])+"px");
                            $("#chatbox"+Chatter.Id).css('top',parseInt( $("#"+Chatter.Id).css('top').split("px")[0]-$("#chatbox"+Chatter.Id).height())   +"px");
                            $("#chatbox"+Chatter.Id).css('z-index',$("#"+Chatter.Id).css('z-index'));
                           // alert($("#chatbox"+Chatter.Id).height());
...
if(isNaN(y)==false)
{
     ...
     $("#chatbox"+Chatter.Id).css('top',parseInt( y-$("#chatbox"+Chatter.Id).height(),10)+"px");     
     $("#chatbox"+Chatter.Id).css('z-index',y);     
     
    var bLeft= parseInt($("#"+Chatter.Id).css('background-position').toString().split(" ")[0].toString().split('px')[0],10);
    //$("#publicFeed").html(bLeft);
     if(bLeft==256){
        bLeft=64;
     }
     else{
